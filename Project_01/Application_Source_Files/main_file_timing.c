...
int main()
{
	...
	u32 Addr;
	volatile unsigned int Data;           // means do not optimizi it
	...
	for (i = 0; i < NUMBER_OF_TRIALS; i++) {
		...
		//uint32_t saif_uint32 = (uint32_t)(rand() & 0xFFFF);                     // a random 32-bit binary number
		//float x_saif_float = (float)rand();                           // floating point random number
		//float y_saif_float = (float)rand();


	    float rand_float = (float)rand() / (float)RAND_MAX;            // generate a random float for printf test
		...
		//REPEAT_40(Data = buffer[Addr]);                                                            // Timing of reading a word from the DDR
		//REPEAT_5(XGpio_DiscreteWrite(&Gpio, LED_CHANNEL, saif_uint32));                                      // Turns on one LED
		//REPEAT_40(x_saif_float = x_saif_float + y_saif_float);


		// ---- The USB part 5:
	    // ---- (a) Timing of writing a floating point number using printf()
	    // always use \n to flush the buffer
		REPEAT_40(printf("%f\n", rand_float));

	    // ---- (b) Timing of writing a string of 10 characters using xil_printf()
		//REPEAT_40(xil_printf("ABCDEFGHIJ\n"));
